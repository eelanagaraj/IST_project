...
def tune_model(X_train, y_train, settings) :
	(optimizer, loss_func, activation, LSTM_in, LSTM_out) = settings
	print("Loading data...")
...
""" extract the partial activations of the training and test data
	...
	of each sentence, grouped by app """
def extract_representation(maxlen, settings, seed=107, test_split=0.2) :
	# load data
	(X,y) = ISTapps.load_apps_shuffled(maxlen, seed)
	X = np.asarray(X)
	y = np.asarray(y)
	...
	model = tune_model(X_train, y_train, settings)

	# training data --> should get a list of activation arrays/lists per app
	...
	assert(len(X_test_rep) == len(y_test))

	return (X_train_rep, y_train_prime), (X_test_rep, y_test), model
...
def early_fusion (X_train_rep, y_train, X_test_rep, y_test) :
...
def late_fusion (X_train_rep, y_train, X_test_rep, y_test) :
	




# store representation for ease of testing:
pkl_file = '/home/enagaraj/extracted_reps.pkl'
if not (os.path.isfile(pkl_file)) :
	(X_train_rep, y_train), (X_test_rep, y_test), model = extract_representation(maxlen, settings, seed=107, test_split=0.2)
	data = dict(X_train_rep=X_train_rep, y_train=y_train, X_test_rep=X_test_rep, y_test=y_test, model_weights=model.get_weights())
	pkl.dump(data, open(pkl_file, 'wb'), protocol=pkl.HIGHEST_PROTOCOL)
else :
	data = pkl.load(open(pkl_file, 'rb'))
	X_train_rep = data['X_train_rep']
	y_train = data['y_train']
	X_test_rep = data['X_test_rep']
	y_test = data['y_test']
	model_weights = data['model_weights']


acc = early_fusion(X_train_rep, y_train, X_test_rep, y_test)




"""
def get_means (X, LSTM_out) :
	fused_set = np.asarray([np.mean(app, axis=0) for app in X])
	avgs = np.zeros((len(X),1,LSTM_out))
	for i,app in enumerate(X) :
		avg = np.zeros((1,LSTM_out))
		for sentence in app :
			avg += sentence
		avg = avg / (len(app) * 1.0)
		avgs[i] = avg
	return fused_set, avgs
"""